...
Matching starts at the root element, rather than the document.  This slightly
differs from twig.  Pretend your XML is C<< <r><x><y>7</y></x></r> >>,
C<$CQ->cute_query('x/y'=>'')> will return C<7>, while Twig (i.e.,
C<$twig->get_xpath("x/y")> would return an empty list (since the document starts
at C</> instead of C<< <r> >>.

CQ will also return C<< <r> >> when matching C<"."> after a parse.

=head2 B<C<$how_you_would_like_it>>
