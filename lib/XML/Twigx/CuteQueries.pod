...
XML::Twigx::CuteQueries - A cute little query language for converting XML to Perl
...
L<XML::Twigx::CuteQueries> L<is a|perlvar/@ISA> L<XML::Twig>, so when this
...
details on the author's perceived unpredictability of L<XML::Simple>).
...
Cute queries are made up of pairs:
    ...
    $what_you_are_requesting => $how_you_would_like_it
...
regular expressions.  Regular expression selectors are matched against each
direct descendant one at a time, while the xpath selectors use Twig's internal
matching mechanisms to flit around the document willy nilly.

Matching starts while sitting on the root element, rather than the Twig.  This
slightly differs from twig.  Pretend your XML is C<< <r><x><y>7</y></x></r> >>,
C<$CQ->cute_query('x/y'=>'')> will return C<7>, while Twig would return an empty
list (since the document starts at C</> instead of C<< <r> >>.

CQ will also return C<< <r> >> when matching C<"."> after a parse.

CQ keeps track of your preference for keys internally.  That is, if you 

=head1 EXAMPLES

For these examples, the following XML source is assumed.
...
Preserving the text in a C<#PCDATA> node does seem to require L</recurse_text>.

    my $r3 = $CQ->cute_query(
        {nofilter_nontags=1, recurse_text=>1},
        '.'=>[x=>'']);
    # $r3 is [' ', 7, ' ', 8]
...
perl(1), L<XML::Twig>, L<XML::Twigx::CuteQueries::Error>
