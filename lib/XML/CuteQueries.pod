...
scalar-type results.  If you'd like to return all the text, use this query
instead.
...
You can also choose to slurp out XML or xhtml using the C<'xml()'en requesting
C<''> results.  If you'd like to return all the text, use this query instead.
...
Lastly, if you want to do some twig powerful things, you can ask for the twig
elements directly:
...
There are several synonyms for this:
    ...
    # all the same as ($x,$y)
    my @same = $CQ->cute_query(p=>'t');
    my @same = $CQ->cute_query(p=>'twig');
    my @same = $CQ->cute_query(p=>'twig()');
...
=item Hans Dieter Pearcey C<< <?????@weftsoar.net> >>

I did not know there wasn't a C<< L<text|scheme:> >>.  Why doesn't Test::Pod
